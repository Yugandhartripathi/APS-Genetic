...
{ 
  for (int i = 0; i < populationSize; i++)
  {
    ...
    for (int j = 0; j < sizeOfEachTeam; j++)
    {
      ...
      //cout<<newOrder.getGeneAtIndex(j).getName()<<" \n";
    }
    //cout<<newOrder.getGeneAtIndex(0).getName()<<" \n";
    chromosomes.push_back(newOrder);
    for(int z=0;z<sizeOfEachTeam;z++)
    {
      chromosomes[i].setGeneAtIndex(z,newOrder.getGeneAtIndex(z));
    }
    cout<<newOrder.getGeneAtIndex(0).getName()<<endl;
  }
  cout<<chromosomes[1].getGeneAtIndex(0).getName();
}
